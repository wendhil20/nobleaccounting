...
<div class="crm-step-panel" id="crm-step-3">
    <p class="text-xs text-gray-400 mb-4">Who's handling this and the contract amount.</p>
...
<div>
    ...
    <div class="relative">
        ...
        <input type="text" inputmode="decimal" id="crm_contract_amount_display" placeholder="0.00"
            value="<?= !empty($_POST['contract_amount']) ? number_format((float) $_POST['contract_amount'], 2) : '' ?>"
            class="w-full pl-7 pr-3 py-2.5 text-sm text-gray-800 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-amber-600 focus:border-amber-600 transition">
        <input type="hidden" name="contract_amount" id="crm_contract_amount"
            value="<?= htmlspecialchars($_POST['contract_amount'] ?? '') ?>">
    </div>
</div>
    </div>
